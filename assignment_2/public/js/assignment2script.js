...
function displayMatches() {
  ...
  const html = matchArray.map(task => {
    ...
    const displayFinal = task.taskName.replace(regex, `<span style="background-color: #FFFF00">${this.value}</span>`);
    return `
      <li>
        <span class="name">${displayFinal}</span>
      </li>
    `;
  }).join('');
  getSearchTaskContent.innerHTML = html;
}

const searchTask = document.getElementById('searchTask');
const getSearchTaskContent = document.getElementById("searchTaskContent");
searchTask.addEventListener('change', displayMatches);
searchTask.addEventListener('keyup', displayMatches);
